...
def shannon_fano(p):
    ...
    for counter, a in enumerate(p): # for every probability in p
        f.append(f[counter] + p[a])
...
def huffman(p):
    ...
    p = [(k, p[a]) for k, a in enumerate(p)]
    ...
    while len(p) > 1:
        ...
        xt.append([-1, [p[0][0], p[1][0]], str(nodelabel)])
        ...
        # print('p[0][0] = %s'%p[0][0])
        xt[p[0][0]][0] = nodelabel
        xt[p[1][0]][0] = nodelabel
        ...
        p.append((nodelabel, p[0][1] + p[1][1]))
        # print('p = %s'%p)
        # print('xt = %s'%xt)
